...
p + geom_boxplot(outlier.size=NA) +  # Turn off outliers, as we plot each datapoint below.
...
adonis(Num_Reads ~ Donor, data=sample_depth_df, method="euclidean")
sink()
```
...
p = ggrare(phylo_post, step = 1000, line_weight=0.4,
           label= NULL, # 'Sample', 
           color='Donor', plot = TRUE, parallel = TRUE, se = FALSE)
p + theme(text = element_text(size=9)) +
ggsave("post_gavage_rarefaction_curves-ANNOTEATED.pdf")
# Plot for publication.
p + theme(legend.position="none", text = element_text(size=9), axis.title = element_text(size=8)) +
...
richness$Donor = sample_data(phylo_post)$Donor  # Add a column?s
